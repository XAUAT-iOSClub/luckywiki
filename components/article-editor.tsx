  ...
  return (
    <form action={formAction} className="flex flex-col flex-1 min-h-0 relative">
      ...
      <section className="flex-1 flex flex-col min-h-0 rounded-[2.5rem] border border-border/40 bg-white/60 dark:bg-zinc-900/60 shadow-2xl overflow-hidden backdrop-blur-2xl">
        {/* Unified Toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-2 p-3 border-b border-border/20 bg-white/40 dark:bg-black/40 backdrop-blur-md shrink-0">
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="ghost"
              type="button"
              className="h-9 px-4 rounded-full text-[10px] font-bold uppercase tracking-tight hover:bg-primary/5 hover:text-primary transition-colors"
            >
              {isUploadingImage ? (
          ...
          <div className="flex items-center gap-1 bg-background/50 p-1 rounded-full border border-border/50">
            <Button
              ...
              className="h-7 px-3 rounded-full text-[10px] font-bold uppercase"
            >
            ...
            <Button
              ...
              className="hidden lg:flex h-7 px-3 rounded-full text-[10px] font-bold uppercase"
            >
              <Layout className="size-3 mr-1.5" />
              {t.common.split}
            </Button>
            <Button
              type="button"
      ...
      {/* Floating Action Bar */}
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 p-2 rounded-full border border-border/50 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-2xl shadow-2xl">
        <Drawer direction="right">
          <DrawerTrigger asChild>
            <Button variant="ghost" size="icon" className="size-11 rounded-full hover:bg-primary/10 hover:text-primary transition-all active:scale-95">
        ...
        <SubmitButton className="rounded-full h-11 px-8 shadow-xl shadow-primary/20 bg-primary hover:shadow-primary/30 active:scale-95 transition-all text-sm font-semibold tracking-tight">
          {submitLabel}
        </SubmitButton>
        
        {(state.error || state.success) && (
          <div className={cn(
            "px-5 py-2.5 rounded-full text-xs font-bold max-w-[200px] truncate shadow-sm",
